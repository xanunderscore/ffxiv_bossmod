...
public class ReplayManagementWindow : UIWindow
{
    ...
    public ReplayManagementWindow(WorldState ws, PlanDatabase planDB, DirectoryInfo logDir, string savedReplaysFile) : base(_windowID, false, new(300, 200))
    {
        _ws = ws;
        _logDir = logDir;
        _config = Service.Config.Get<ReplayManagementConfig>();
        _manager = new(planDB, logDir.FullName, savedReplaysFile);
        _subscriptions = new
        (
