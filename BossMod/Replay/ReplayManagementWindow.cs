...
public class ReplayManagementWindow : UIWindow
{
    ...
    public ReplayManagementWindow(WorldState ws, RotationDatabase rotationDB, DirectoryInfo logDir, string replayHistoryFile) : base(_windowID, false, new(300, 200))
    {
        _ws = ws;
        _logDir = logDir;
        _config = Service.Config.Get<ReplayManagementConfig>();
        _manager = new(rotationDB, logDir.FullName, replayHistoryFile);
        _subscriptions = new
        (
