...
class OpList(Replay replay, ModuleRegistry.Info? moduleInfo, IEnumerable<WorldState.Operation> ops, Action<DateTime> scrollTo)
{
    ...
    private string OpName(WorldState.Operation o)
    {
        return o switch
        {
            ...
            ActorState.OpMount op => $"Mount: {ActorString(op.InstanceID, op.Timestamp)} = {op.Value} {Service.LuminaRow<Lumina.Excel.GeneratedSheets.Mount>(op.Value)?.Singular ?? "<unknown>"}",
