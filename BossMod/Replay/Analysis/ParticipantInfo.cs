...
class ParticipantInfo : CommonEnumInfo
{
    ...
    public void DrawContextMenu()
    {
        ...
        if (ImGui.MenuItem("Generate missing enum values for boss module"))
        {
            ...
            {
                var (member, val) = EnumMemberString(oid, data);
                sb.AppendLine($"{member} = {val}");
            }
    ...
    private static bool IsIgnored(Replay.Participant p) => p.Type is ActorType.Player or ActorType.Pet or ActorType.Chocobo or ActorType.Area or ActorType.Treasure or ActorType.DutySupport || p.WasAlly;
    ...
    private (string Member, string Value) EnumMemberString(uint oid, ParticipantData data, string? forcedName = null)
    {
    ...
    private StringBuilder AddOIDEnum(StringBuilder sb, uint forcedBossOID = 0)
    {
        ...
        {
            var i = 0;
            var (member, val) = EnumMemberString(oid, data, oid == forcedBossOID ? "Boss" : null);
            var tmpKey = member;
            while (!members.TryAdd(tmpKey, val))
                tmpKey = $"{member}{++i}";
        }
