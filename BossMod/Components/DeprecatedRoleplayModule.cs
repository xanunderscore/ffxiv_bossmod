...
public abstract class RotationModule<R>(BossModule module) : BossComponent(module) where R : StatelessRotation
{
    private readonly R _rotation = New<R>.Constructor<WorldState>()(module.WorldState);
    public sealed override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints, float maxCastTime) => _rotation.Execute(actor, hints, maxCastTime);
}

public abstract class DeprecatedRoleplayModule(BossModule module) : BossComponent(module)
{
